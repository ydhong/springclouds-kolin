...
import com.umeet.conference.contants.Response
import com.umeet.conference.model.RoleAndPrivilege
import com.umeet.conference.model.User
import com.umeet.conference.service.RoleAndPrivilegeService
...
@RestController
@RequestMapping("/umeet/roleAndPrivilege")
@Api(value = "RoleController", description = "系统角色权限接口")
class RoleAndPrivilegeController {
    ...
    @ResponseBody
    fun query(record: RoleAndPrivilege) :String {
        val RolePage = roleAndPrivilegeService!!.getPage(record)
        return FastJsonUtils.resultSuccess(200, "成功", RolePage)
    }


    @ApiOperation(value = "查询" , httpMethod = "GET")
    @RequestMapping("queryRoleAndPrivilegeBySessionId")
    @ResponseBody
    fun queryRoleAndPrivilegeBySessionId(request: HttpServletRequest) :Any {
        val session: HttpSession = request.getSession();
        //检查账号有效性
        var sessionUser = session.getAttribute(Constant.LOGIN_ADMIN_USER) as User?;
        val roleAndPrivileges= roleAndPrivilegeService!!.getPrivatesByUser(sessionUser!!)
        return Response.resultSuccess(200, "成功", roleAndPrivileges)
    }
    ...
    @ResponseBody
    fun save(@RequestBody(required = false) record: RoleAndPrivilege) :String {
        val row = roleAndPrivilegeService!!.save(record);
        if(row == 0) {
    ...
    fun delete(@PathVariable id: Int): String {
        val row = roleAndPrivilegeService!!.deleteByPrimaryKey(id);
        if(row == 0) {
    ...
    fun deletes(@RequestBody  params: Map<String, Object>): String {
        ...
        try {
            for (id in ids) {
                roleAndPrivilegeService!!.deleteByPrimaryKey(id);
            }
        } catch (e: Exception) {
            return FastJsonUtils.resultError(-200, "操作失败", null);
        }
    ...
    fun enables(@RequestBody  params: Map<String, Object>): String {
        ...
        try {
            for (id in ids) {
                val record: RoleAndPrivilege =  RoleAndPrivilege();
                record.id=id
                roleAndPrivilegeService!!.updateByPrimaryKeySelective(record);
            }
        } catch (e: Exception) {
