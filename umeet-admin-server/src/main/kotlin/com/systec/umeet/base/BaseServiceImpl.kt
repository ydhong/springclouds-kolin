...
import com.systec.umeet.contants.Condition
import com.systec.umeet.contants.ConditionType
...
abstract class BaseServiceImpl< T: BaseEntity> : BaseService<T>{
	 ...
	 * @return int 影响行数
	 */
	override fun update(record: T): Int {
		var count = 0;
	 ...
	 */
	override fun selectPage(pageNum: Int, pageSize: Int, record: T?): ResponsePage<T> {
		PageHelper.startPage<T>(pageNum, pageSize)
		return ResponsePage<T>(getMapper().select(record));
	}
	 ...
	 * @return PageInfo 分页对象
	 */
	override fun  selectPageByCondicion(pageNum: Int, pageSize: Int, record: T?,conditions: MutableList<Condition>,orderSqlStr:String):PageInfo<T> {
		val example = Example(record?.javaClass,false)
		val criteria = example.createCriteria()
		try {
			for(condition in conditions){
				if(ConditionType.EQUAL.equals(condition.type)){
					criteria.andEqualTo(condition.key, condition.value)
				}else if(ConditionType.LIKE.equals(condition.type)){
					criteria.andLike(condition.key, condition.value)
				}else if(ConditionType.IN.equals(condition.type)){
					criteria.andIn(condition.key, condition.values)
				}else if(ConditionType.BETWEEN.equals(condition.type)){
					criteria.andBetween(condition.key,condition.beginValue,condition.endValue)
				}
			}
			example.orderByClause= orderSqlStr
			PageHelper.startPage<T>(pageNum, pageSize);
	 ...
	 * @return List 列表
	 */
	override open fun  selectByCodition(record: T?, conditions: MutableList<Condition>,orderSqlStr: String?): List<T>{
		val example: Example = Example(record?.javaClass,false);
		val criteria: Criteria = example.createCriteria();
		try {
			for(condition in conditions){
